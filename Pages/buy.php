        ...
        <form action="buyinfo.php" method="post">
            ...
            <div class="form-group">
                ...
                <select name='selectemployee'>
                    ...
                    <?php
                    include "buyinfo.php";  // Using database connection file here
                    $records = mysqli_query($db, "SELECT e_id, e_name From employee");  // Use select query here

                    while ($data = mysqli_fetch_array($records)) {
                        echo "<option value='" . $data['e_id'] . "'>" . $data['e_name'] . "</option>";  // displaying data in option menu
                    }
                    ?>
                </select>
            </div>
